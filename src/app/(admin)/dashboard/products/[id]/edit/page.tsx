...
import { ArrowLeft, Upload, X, Save, Loader2, Trash2, Plus } from 'lucide-react'
...
export default function EditProductPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setForm({
          ...
          wickType: product.specifications?.wickType || 'cotton',
          ...
          diameter: product.specifications?.dimensions?.diameter?.toString() || '',
          containerMaterial: product.specifications?.containerMaterial || 'glass',
          careInstructions: product.careInstructions?.map((ci: any) => ci.instruction || ci) || [],
  ...
  async function fetchCollections() {
    try {
      ...
      if (response.ok) {
        ...
        // Only real collections from the DB (valid ids). Never inject placeholder
        // ids — a non-existent id makes the product save fail.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload any new images in parallel (much faster than one-by-one)
      const uploadedImageIds: string[] = (
        await Promise.all(
          newImages.map(async (image) => {
            ...
            throw new Error(errData.error || `Failed to upload ${image.name}`)
          })
        )
      )
